...
O projeto foi reconstruído como um fluxo de dados reproduzível, e não como uma transposição de slides. A análise combina **Python, Pandas, SQL, SQLite, Plotly e Streamlit**, com fontes rastreáveis e validação automatizada.

O dashboard está organizado em cinco frentes:
...
As variações e relações entre indicadores são calculadas no produto, evitando conclusões fixadas manualmente na interface.
...
│   ├── 03_risk.sql
│   └── 04_customer.sql
└── .github/workflows/                  # validação e atualização dos dados processados
```
...
As consultas em `sql/` tornam explícitas perguntas de negócio que também aparecem no dashboard:
...
- compilação de `app.py` e dos módulos Python;
...
- [ ] versão pública em GitHub Pages usando os mesmos datasets
- [ ] preview visual e link público no topo do README
